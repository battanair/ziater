...
const EditPlayAwards2 = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchPremio = async () => {
        const docRef = doc(db, 'premios_obras', id);
  ...
  const handleSubmit = async () => {
    try {
      if (id) {
        await updateDoc(doc(db, 'premios_obras', id), premio);
        alert('Premio actualizado');
      } else {
        const docRef = await addDoc(collection(db, 'premios_obras'), premio);
        alert('Premio guardado');
        navigate(`/editarpremio2/${docRef.id}`);
      }
      navigate(-1);
    } catch (error) {
  ...
  return (
    <Box>
      ...
      <Typography variant="h6" gutterBottom>
        {id ? 'Editar Premio de Obra' : 'Nuevo Premio para la Obra'}
      </Typography>
      ...
      <TextField
        label="Galardón"
        value={premio.galardon_obra}
        onChange={(e) => handleChange('galardon_obra', e.target.value)}
        fullWidth
        sx={{ mb: 2 }}
      />
      ...
      <TextField
        ...
        type="number"
        value={premio.anio_premobr}
        onChange={(e) => handleChange('anio_premobr', e.target.value)}
        fullWidth
        sx={{ mb: 2 }}
      />
...
export default EditPlayAwards2;